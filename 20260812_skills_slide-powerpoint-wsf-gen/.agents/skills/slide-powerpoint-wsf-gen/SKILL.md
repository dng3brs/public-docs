...
9. プロジェクトフォルダ内のファイルへの参照は、`WScript.ScriptFullName`が配置されたフォルダをプロジェクトフォルダと想定し、プロジェクトフォルダからの相対パスで参照する。
10. 絶対パスでのファイル参照は不可
11. `On Error`などエラーハンドリングの処理は実装しない。

---

## 作業手順

1. スライドの作成を行う前に、[スライド作成ガイドライン](references/slide-guideline.md) をすべて読み、スライド作成作業全般に適用する
2. 高確度に推測できない情報は質問する：
...
3. 描写する内容を検討する
   ...
   - レイアウトを決める
4. 指定されたファイル名で直接wsfを書き出す。
   - `.wsf` 拡張子で作成する。
   - 指定がない限り、追加の成果物を作成しない。
5. 完了を報告する前に、結果を検証すること：
